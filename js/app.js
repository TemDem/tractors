 ...
 */
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".equipment__slider_1") && document.querySelector(".equipment__slider_2")) {
                const equipmentSlider1 = new swiper_core_Swiper(".equipment__slider_1", {
                    ...
                    speed: 800,
                    navigation: {
                        prevEl: ".top-section__arr_prev",
                        nextEl: ".top-section__arr_next"
                    },
                    on: {}
                });
                const equipmentSlider2 = new swiper_core_Swiper(".equipment__slider_2", {
                    modules: [ Navigation, Controller ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 2,
                ...
                equipmentSlider1.controller.control = equipmentSlider2;
                equipmentSlider2.controller.control = equipmentSlider1;
            }
        ...
        if (popupBtns.length > 0) popupBtns.forEach((btn => {
            btn.addEventListener("click", (function(e) {
                if (btn.dataset.title) {
                    const title = btn.dataset.title;
                    const popup = document.querySelector(btn.dataset.popup);
                    if (popup.querySelector(".info-popup__title")) popup.querySelector(".info-popup__title").textContent = title;
                    if (popup.querySelector(".info-popup__btn")) popup.querySelector(".info-popup__btn").dataset.title = title;
                    if (popup.querySelector(".hidden")) popup.querySelector(".hidden").value = title;
                }
                if (btn.dataset.images) {
                    const images = btn.dataset.images.split(",");
                    const popup = document.querySelector(btn.dataset.popup);
                    const popupSliderWrapper = popup.querySelector(".popup-slider__wrapper");
                    const popupThumbsWrapper = popup.querySelector(".thumbs__wrapper");
                    let popupSliderHtml = "";
                    let popupThumbsHtml = "";
                    images.forEach(((image, i) => {
                        if (i == images.length - 1) return;
                        popupSliderHtml += `<div class="popup-slider__slide swiper-slide"><img src="${image}" alt="image"></div>`;
                    }));
                    images.forEach(((image, i) => {
                        if (i == images.length - 1) return;
                        popupThumbsHtml += `<div class="thumbs__slide swiper-slide"><img src="${image}" alt="image"></div>`;
                    }));
                    if (popupSliderWrapper.children.length > 0) {
                        const slides = Array.from(popupSliderWrapper.children);
                        slides.forEach((slide => {
                            slide.remove();
                        }));
                    }
                    if (popupThumbsWrapper.children.length > 0) {
                        const slides = Array.from(popupThumbsWrapper.children);
                        slides.forEach((slide => {
                            slide.remove();
                        }));
                    }
                    popupSliderWrapper.insertAdjacentHTML("afterbegin", popupSliderHtml);
                    popupThumbsWrapper.insertAdjacentHTML("afterbegin", popupThumbsHtml);
                }
                if (btn.dataset.btn) {
                    const popup = document.querySelector(btn.dataset.popup);
                    const btnTextEl = popup.querySelector(".info-popup__text");
                    btnTextEl.textContent = btn.dataset.btn;
                }
                if (btn.dataset.price) {
                    const popup = document.querySelector(btn.dataset.popup);
                    const priceEl = popup.querySelector(".info-popup__price");
                    priceEl.textContent = btn.dataset.price;
                }
                if (btn.dataset.adv) {
                    const advArrs = btn.dataset.adv.split(";;");
                    const popup = document.querySelector(btn.dataset.popup);
                    const characteristics = popup.querySelector(".info-popup__characteristics");
                    let characteristicsHtml = "";
                    if (characteristics.children.length > 0) {
                        const characteristicsArr = Array.from(characteristics.children);
                        characteristicsArr.forEach((characteristic => {
                            characteristic.remove();
                        }));
                    }
                    console.log(advArrs);
                    advArrs.forEach(((adv, i) => {
                        if (advArrs.length - 1 > i) {
                            const advArr = adv.split("||");
                            characteristicsHtml += `<div class="info-popup__characteristics-row">\n\t\t\t\t\t\t\t\t<div class="info-popup__left">${advArr[0]}</div>\n\t\t\t\t\t\t\t\t<div class="info-popup__right">${advArr[1]}</div>\n\t\t\t\t\t\t\t</div>`;
                        }
                    }));
                    characteristics.insertAdjacentHTML("afterBegin", characteristicsHtml);
                }
            }));
        }));
        document.addEventListener("afterPopupClose", (function(e) {
            if (document.documentElement.classList.contains("lock") && !document.documentElement.classList.contains("popup-show")) document.documentElement.classList.remove("lock");
        }));
        const scrollLink = document.querySelectorAll("._scroll a");
        if (scrollLink.length > 0) scrollLink.forEach((link => {
            const goEl = "#" + link.href.split("#")[1];
            link.setAttribute("data-goto", goEl);
        }));
        isWebp();
        menuInit();
        pageNavigation();
